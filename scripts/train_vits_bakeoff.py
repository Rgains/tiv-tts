#!/usr/bin/env python3
"""Run a bounded, character-level official VITS trial on the derived Tiv subset."""
...
def build_configuration(
    ...
    return VitsConfig(
        ...
        run_name=f"tiv_vits_{steps}_steps",
        run_description="Bounded Tiv character-level VITS comparison run.",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    requested_steps = args.steps or int(
        bakeoff["comparison"]["local_max_steps"]
    )
    ...
    if requested_steps > len(train_samples):
        raise ValueError(
            f"Requested {requested_steps} steps but only "
            f"{len(train_samples)} one-sample batches are available."
        )
    train_samples = train_samples[:requested_steps]
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("outputs/bakeoff/vits")
    )
